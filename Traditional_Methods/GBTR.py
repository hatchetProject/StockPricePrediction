...
class Gradient_Boosting_Regressor:
    def __init__(self, loss='ls', lr=0.1, n_estimate=100, depth=3, vb=1):
        self.train_set = None
        self.train_label = None
        self.validation_set = None
        self.validation_label = None
        self.test_set = None
        self.model = GradientBoostingRegressor(loss=loss, learning_rate=lr, n_estimators=n_estimate, max_depth=depth, verbose=vb)
...
if __name__=="__main__":
    ...
    reframed_train = clip_data_by_time(reframed_train, RTime)
    train_x,train_y = training_dataset(reframed_train)
    print (train_x.shape)
    print ("OK")

    GBC_Model = Gradient_Boosting_Regressor("ls", 0.1, 300, 10)
    print ("Begin Training")
    mse_loss = GBC_Model.train(train_x, train_y)
    print ("Training Finished")
    test_dataset =load_test_data()
    scaled_dataset, scaler = testing_dataset(test_dataset)
    scaled_dataset = scaled_dataset.reshape((1000, 70))
    test_result = GBC_Model.test(scaled_dataset, scaler)
